...
interface NavbarProps {
  currentView: 'landing' | 'proposal' | 'packages';
  onNavigate: (view: 'landing' | 'proposal' | 'packages') => void;
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const [activeSection, setActiveSection] = useState<'services' | 'estimator' | 'packages'>('services');
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (estimatorEl) {
        ...
        // If estimator is within viewport (top half of screen)
        if (rect.top <= 200 && rect.bottom >= 150) {
          setActiveSection('estimator');
          return;
        }
      }
      // Otherwise in top services section
  ...
  const handleNavClick = (section: 'services' | 'estimator' | 'packages') => {
